...
一个变量被指定了某个数据类型，如果不经过强转，永远就是这个数据类型

> 强类型语言速度上可能逊色于弱类型定义的语言，但是严谨性能够有效避免许多错误

> ![语言类型](https://img.php.cn/upload/article/000/000/024/75a0bfabf94b19c86368d5f06e8c647e-0.png)
...
重载发生在同一个类中，若多个方法之间**方法名相同，参数列表不同**，则它们构成重载关系。
...
Java5提供，允许调用方法时传入不定长度的参数，本质上是基于数组的实现
